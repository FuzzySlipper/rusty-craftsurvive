...
- The layered masonry test changes only the west-wall bay at z=-2..2. Separate brick/backing meshes avoid per-triangle brick/mortar classification there; other walls retain existing material seams and DC defects. This measured trade exchanges fewer triangles for more retained pieces and draws. It is not a general topology fix or batching solution; see [the test result](layered-masonry-test.md).
