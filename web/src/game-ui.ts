...
export function mountCraftSurviveUi(root: HTMLElement, context: RustyApplicationUiContext): RustyApplicationUiOwner {
  ...
  const garden = gardenEnabled ? new RuntimeVoxelSpriteGarden(
    ...
    (value) => {
      ...
      panel.querySelector<HTMLSelectElement>('[data-lab-ghost-shell]')!.value = value.ghostShellMode;
      panel.querySelector<HTMLInputElement>('[data-lab-ghost-shell-epsilon]')!.value = String(value.ghostShellDepthEpsilon);
      const shellEpsilonOutput = panel.querySelector<HTMLOutputElement>('[data-lab-ghost-shell-epsilon-value]')!;
      const shellQuantizationHalfStep = value.ghostShellDepthQuantizationStep === null
        ? 'n/a'
        : (value.ghostShellDepthQuantizationStep * 0.5).toFixed(3);
      shellEpsilonOutput.value = `${value.ghostShellDepthEpsilon.toFixed(2)} → ${value.ghostShellEffectiveDepthEpsilon?.toFixed(3) ?? 'n/a'} eff.`;
      shellEpsilonOutput.title = `${value.ghostShellDepthEpsilon.toFixed(2)} tolerance + ${shellQuantizationHalfStep} quantization half-step = ${value.ghostShellEffectiveDepthEpsilon?.toFixed(3) ?? 'n/a'} effective tolerance`;
