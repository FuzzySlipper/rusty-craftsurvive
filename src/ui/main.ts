...
import { mountRopes } from './ropes.js';
...
export function mountProductUi(root: Element): Readonly<{ dispose(): void }> {
  ...
  const ropes = mountRopes(panel, transport);
  const workbench = mountProcgenWorkbench(panel, transport);
  const ghost = mountGhostSettings(panel, transport);
  root.append(panel);
  return Object.freeze({ dispose: () => {
    disposed = true; ropes.dispose(); courtyard.dispose(); workbench.dispose(); ghost.dispose(); debugPanel?.dispose(); metrics.dispose(); panel.remove();
  } });
}
